...
import {
  format, subDays, addDays, setMinutes, setSeconds, setHours, setMilliseconds, isBefore, isEqual, parseISO,
} from 'date-fns';
  ...
  useEffect(() => {
    async function fetchSchedule() {
      ...
      const data = ranges.map((hour) => {
        ...
        console.tron.log(timeZoneDate);
        return {
          time: `${hour}:00h`,
          past: isBefore(timeZoneDate, date),
          appointment: response.data.find((a) => isEqual(parseISO(a.date), timeZoneDate)),
        };
      });
